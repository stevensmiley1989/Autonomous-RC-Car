...
class SingleInference_YOLOV7:
    ...
    def swap_names(self):
        '''
        If you want to switch the label on the fly of a model output (i.e. model has "person" but you want "John" instead.

        INPUT example:
            names=["dog","cat","person"]
            replace_dic={"person":"John"}
        OUTPUT example:
            names=["dog","cat","John"]
            
            '''
        if type(self.replace_dic)!=type(None) and type(self.replace_dic)==type({}):
            new_names=[]
            for name_i in self.names:
                if name_i in list(self.replace_dic.keys()):
                    name_i=self.replace_dic[name_i]
                new_names.append(name_i)
            ...
            for i,det in enumerate(self.outputs):
                if len(det):
                    ...
                    for j,(*bboxes,score,cls_id) in enumerate(reversed(det)):
                        ...
                        self.predicted_bboxes_PascalVOC.append([name,x0,y0,x1,y1,score,image.shape[0],image.shape[1]]) #PascalVOC annotations
...
if __name__=='__main__':  
    ...
    \t name,x0,y0,x1,y1,score,height,width\n
    {app.predicted_bboxes_PascalVOC}''')
